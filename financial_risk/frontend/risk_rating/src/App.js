...
const FinancialRiskForm = () => {
  ...
  return (
    <div style={{ padding: '50px' }}>
      <h1>Financial Risk Prediction</h1>
      <Form onFinish={handleFormSubmit} layout="vertical">
        
        {/* Age */}
        <Form.Item label="Age" name="age" rules={[{ required: true, message: 'Please enter age' }]}>
          <InputNumber min={18} max={100} onChange={(value) => handleChange(value, 'age')} />
        </Form.Item>
        
        {/* Income */}
        <Form.Item label="Income" name="income" rules={[{ required: true, message: 'Please enter income' }]}>
          <InputNumber min={0} onChange={(value) => handleChange(value, 'income')} />
        </Form.Item>
        
        {/* Credit Score */}
        <Form.Item label="Credit Score" name="credit_score" rules={[{ required: true, message: 'Please enter credit score' }]}>
          <InputNumber min={300} max={850} onChange={(value) => handleChange(value, 'credit_score')} />
        </Form.Item>
        
        {/* Loan Amount */}
        <Form.Item label="Loan Amount" name="loan_amount" rules={[{ required: true, message: 'Please enter loan amount' }]}>
          <InputNumber min={0} onChange={(value) => handleChange(value, 'loan_amount')} />
        </Form.Item>

        {/* Years at Current Job */}
        <Form.Item label="Years at Current Job" name="years_at_current_job" rules={[{ required: true, message: 'Please enter years at current job' }]}>
          <InputNumber min={0} max={50} onChange={(value) => handleChange(value, 'years_at_current_job')} />
        </Form.Item>
        
        {/* Debt-to-Income Ratio */}
        <Form.Item label="Debt-to-Income Ratio" name="debt_to_income_ratio" rules={[{ required: true, message: 'Please enter debt-to-income ratio' }]}>
          <InputNumber min={0} max={100} onChange={(value) => handleChange(value, 'debt_to_income_ratio')} />
        </Form.Item>

         {/* Assets Value */}
         <Form.Item label="Assets Value" name="assets_value" rules={[{ required: true, message: 'Please enter assets value' }]}>
          <InputNumber min={0} onChange={(value) => handleChange(value, 'assets_value')} />
        </Form.Item>

         {/* Marital Status Change */}
         <Form.Item label="Marital Status Change" name="marital_status_change" rules={[{ required: true, message: 'Please enter marital status change' }]}>
          <InputNumber min={0} onChange={(value) => handleChange(value, 'marital_status_change')} />
        </Form.Item>

        {/* Previous Defaults */}
        <Form.Item label="Previous Defaults" name="previous_defaults" rules={[{ required: true, message: 'Please enter number of previous defaults' }]}>
          <InputNumber min={0} onChange={(value) => handleChange(value, 'previous_defaults')} />
        </Form.Item>
        
        {/* Number of Dependents */}
        <Form.Item label="Number of Dependents" name="number_of_dependents" rules={[{ required: true, message: 'Please enter number of dependents' }]}>
          <InputNumber min={0} max={10} onChange={(value) => handleChange(value, 'number_of_dependents')} />
        </Form.Item>
        
        {/* Gender */}
        <Form.Item label="Gender" name="gender" rules={[{ required: true, message: 'Please select gender' }]}>
          <Select onChange={(value) => handleChange(value, 'gender')}>
        ...
        {/* Education Level */}
        <Form.Item label="Education Level" name="education_level" rules={[{ required: true, message: 'Please select education level' }]}>
          <Select onChange={(value) => handleChange(value, 'education_level')}>
        ...
        {/* Marital Status */}
        <Form.Item label="Marital Status" name="marital_status" rules={[{ required: true, message: 'Please select marital status' }]}>
          <Select onChange={(value) => handleChange(value, 'marital_status')}>
        ...
        {/* Loan Purpose */}
        <Form.Item label="Loan Purpose" name="loan_purpose" rules={[{ required: true, message: 'Please select loan purpose' }]}>
          <Select onChange={(value) => handleChange(value, 'loan_purpose')}>
        ...
        {/* Employment Status */}
        <Form.Item label="Employment Status" name="employment_status" rules={[{ required: true, message: 'Please select employment status' }]}>
          <Select onChange={(value) => handleChange(value, 'employment_status')}>
        ...
        {/* Payment History */}
        <Form.Item label="Payment History" name="payment_history" rules={[{ required: true, message: 'Please select payment history' }]}>
          <Select onChange={(value) => handleChange(value, 'payment_history')}>
        ...
        <Form.Item label="Country" name="country" rules={[{ required: true, message: 'Please select country' }]}>
          <Select
            ...
            onSearch={handleCountrySearch}  // Auto-suggest handler
      ...
      {predictionResult && (
        <Card title="Prediction Result" style={{ marginTop: '20px' }}>
          <p>Status: {predictionResult.status}</p>
          <p>Risk Score: {predictionResult.risk}</p>
        </Card>
      )}
    </div>
  );
